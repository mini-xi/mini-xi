...
SELECT CONCAT (menu_price, '원') FROM tbl_menu; -- 이렇게 하는거보다 아래 코드처럼
...
-- LTRIM(문자열), RTRIM(문자열) LTRIM은 왼쪽 공백 제거, RTRIM은 오른쪽 공백 제거
-- TRIM(문자열), TRIM(방향 자를_문자열 FROM 문자열)
SELECT LTRIM('   김    왼쪽   '), RTRIM('오른쪽    김    ');
SELECT TRIM('      MariaDB      '), TRIM(BOTH '@' FROM '@#@@MariaDB@@@@'); -- Both를 쓰면 왼쪽 오른쪽에서 다른 문자 나올때까지 그걸 지운다
...
SELECT RPAD(CONCAT(menu_name, '의 가격은', menu_price, '입니다.') , 30, '@') FROM tbl_menu;

-- FORMAT(슷자, 소수점 자릿수)
SELECT FORMAT(12312312231, 3); -- 화폐 표기할 때 이런거 쓰면 좋음

-- BIN(숫자), OCT(숫자), HEX(숫자)
SELECT BIN(65), OCT(65), HEX(65);

-- REPEAT(문자열, 횟수)
SELECT REPEAT('재밌다잉 ', 5);

-- REPLACE(문자열, 찾을 문자열, 바꿀 문자열)
SELECT REPLACE('Ave 마리아DB', '마리아', 'Maria');
...
SELECT CONV('B', 16, 10), CONV('A', 16, 2);

-- MOD(숫자1, 숫자2) -- 나머지 구하기
SELECT MOD(10, 3), 10%3;

-- POW(숫자1, 숫자2), SQRT(숫자) -- 제곱 구하기, 제곱근 구하기
...
-- SIGN(숫자)  -- 양수면 1, 0이면 0, 음수면 -1 출력
SELECT SIGN(10.1), SIGN(0), SIGN(-1.1);

-- TRUNCATE(숫자, 정수) -- 숫자에서 내가 날리고 싶은걸 정수로 나타내라
SELECT TRUNCATE(12345.12345, 2), TRUNCATE(12345.12345, -2);

-- 3. 날짜 및 시간 관련 함수
-- ADDDATE(날짜, 차이), SUBDATE(날짜, 차이)
SELECT ADDDATE('2023-02-01', INTERVAL 28 DAY), ADDDATE('2020-02-01', 28); -- 윤년도 계산해준다
SELECT SUBDATE('2020-02-01', INTERVAL 1 DAY), SUBDATE('2020-02-01', 1);

SELECT ADDDATE('2023-02-01', INTERVAL 28 MONTH);
...
SELECT DAY('2023-12-28');  -- 암시적 형변환
SELECT DAY(CAST('2023-12-28' AS DATE)); -- 명시적 형변환
...
SELECT DATEDIFF('2023-12-28', '2024-06-14'); -- 최근일수록 크다고 판별한다
SELECT TIMEDIFF('18:00:00', CURTIME()) AS '수업 끝날 때까지 걸리는 시간';

-- DAYOFWEEK(날짜), MONTHNAME(날짜), DAYOFYEAR(날짜)
SELECT DAYOFWEEK(CURDATE()), MONTHNAME(CURDATE()), DAYOFYEAR(CURDATE());

-- LAST_DAY(날짜) -- 달의 마지막 날을 뽑아줌
...
-- MAKETIME(시, 분, 초);
SELECT MAKETIME(17, 50, 01) AS '행복한 시간';

-- QUARTER(날짜)
SELECT QUARTER('2023-12-28');

-- TIME_TO_SEC(시간)
SELECT TIME_TO_SEC(CURTIME()), TIME_TO_SEC('0:0:0'); -- 오늘 흐른 시간