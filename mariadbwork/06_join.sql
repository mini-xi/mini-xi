...
SELECT
	  ...
	  , category_name
	FROM tbl_menu
	JOIN tbl_category ON (tbl_mmenu.category_code = tbl_category.category_code);
...
SELECT
	  ...
	  , category_name
	FROM tbl_menu a
	JOIN tbl_category b ON (a.category_code = b.category_code); -- 두 테이블이 동일한 컬럼일시 별칭 필수
	
SELECT
		 menu_name
	  , category_name
	  , a.category_code -- select절도 동일한 컬럼은 별칭을 필수로 달아야 합니다.
	FROM tbl_menu a
	JOIN tbl_category b ON (a.category_code = b.category_code);
...
-- c. 1번 메뉴의 메뉴 카테고리가 없는 ㅅ당태에서 inner join vs left join 비교하기

SELECT
		 *
	FROM tbl_menu a
	inner JOIN tbl_category b USING (category_code) ORDER BY 1;
...
SELECT
		 *
	FROM tbl_menu a
	JOIN tbl_category b USING (category_code); -- =JOIN tbl_category b ON (a.category_code = b.category_code);
