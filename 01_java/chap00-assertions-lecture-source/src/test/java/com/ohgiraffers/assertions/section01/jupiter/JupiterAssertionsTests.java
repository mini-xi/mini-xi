...
//        Assertions.assertEquals(expected, result, 1);     // 1넣은건 오차범위(+- 1), 그래서 int expected = 31해도 테스트 통과됨
//        Assertions.assertEquals(expected, result, "이건 정확해야해!");    // 오류 메세지를 출력한다.
    }
    ...
    void testAssertNotEqualsWithInstances() {    // 앞에 안 적는 이유는 default로 해도되고 접근제어자가 같은 패키지라고 인식함
