...
public class Application3_1 {
    ...
    public static int solution(String input) throws IOException{
        ...
        int[][] time = new int[N][2];                        // (회의 갯수 * (시작시간 + 종료시간) 일차원 배열)인 이차원 배열, (회의의 갯수만큼의 크기)
        StringTokenizer str;
        for (int i = 0; i < N; i++) {
        ...
        /* 설명.
         *  greedy 알고리즘으로 접근하자면 하나의 회의실에서 가장 많은 회의를 열기 위해서는
        ...
        * */
        Arrays.sort(time, (o1, o2) -> {
            ...
            /* 설명. 종료시간이 같지 않을 때는 종료 시간이 이른 회의부터 정렬되도록 */
            return o1[1] - o2[1];
        });
        ...
        for (int i = 0; i < N; i++) {
            ...
            if(end <= time[i][0]){
                ...
                /* 설명. 조건을 만족하면 회의의 종료 시간으로 end 변수 갱신 */
                end = time[i][1];
                max_count++;
            }
        }
