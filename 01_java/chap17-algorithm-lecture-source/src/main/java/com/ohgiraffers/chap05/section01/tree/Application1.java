...
public class Application1 {
    ...
    public static String solution(String input) throws IOException {
        ...
        sb.delete(0, sb.length());     // 테스트 코드 케이스가 여러 개여서 새로운 예제로 테스트하기 위함
        ...
        /* 설명. list[i] 배열 안에 ArrayList 객체를 담는다. */
        for (int i = 1; i < N + 1; i++) {
    ...
    public static void dfs(int parentNode){
        ...
        /* 설명. 입력 받은 값을 통해 생성된 list로부터 관련 있는 노드들을 확인하기 위한 반복문 */
        for (int node: list[parentNode]) {

            /* 설명. 방문한 적이 없다면 자식 노드이다.(루트부터 방문을 체크하며 연관된 것들을 따라 내려 왔기 때문) */
            if(!isVisit[node]){
