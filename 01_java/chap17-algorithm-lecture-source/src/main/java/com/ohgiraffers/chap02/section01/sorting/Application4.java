 ...
 *   시간 복잡도는 O(nlogn)이지만 최악의 경우는 O(n^2)이 될 수도 있다.
* */
...
public class Application4 {
    ...
    private static int process(int left, int right, int[] arr) {
        ...
        while (true) {
            ...
            /* 설명. pivot 위치에 있는 값보다 크면서 lo 포인터의 위치보다 크거나 같은 위치이면 계속 감소한다. */
            do {
                hi--;
            } while(arr[hi] > pivot && lo <= hi);

            /* 설명. lo 포인터의 위치가 hi 포인터와 앞지르려고 하면 hi 위치를 반환한다.(분할 기준 반환) */
            if(lo >= hi){
                return hi;
            }
            swap(arr, lo, hi);
