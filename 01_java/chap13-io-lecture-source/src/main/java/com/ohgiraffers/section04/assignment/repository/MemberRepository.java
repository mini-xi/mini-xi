...
/* 설명. 데이터가 입출력을 위해 만들어지며 성공 실패 결과를 반환하는 클래스 */
...
//        members.add(new Member(3, "user03", "pass03", 15, new String[]{"음악감상", "독서", "명상"}, BloodType.AB));
    ...
    private void saveMembers(ArrayList<Member> members) {
        ...
        try {
            ...
            /* 설명. 회원이 담긴 ArrayList<> 갯수 만큼 출력하기 */
...
//            return;            // return 해도 finally가 먼저 되고 리턴함
        } finally {
            try {
...
//        return latestMember.getMemNo();                                 // 그 회원의 번호
    ...
    /* 기존 회원(객체)에 이어서 파일 출력을 하고 insert(추가)한 객체의 수를 반환(feat. DML 작업의 결과는 int) */
