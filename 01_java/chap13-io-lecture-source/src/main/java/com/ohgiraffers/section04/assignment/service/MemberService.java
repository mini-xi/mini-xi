...
public class MemberService {
    ...
    public void selectAllMembers() {
        ...
        if (!selectedMembers.isEmpty()) {       // 회원이 한명이라도 조회 된다면
            System.out.println("======= service까지 잘 반환되어 나오나 확인");
            for (Member m : selectedMembers){
                System.out.println(m);
            }

            return;                 // 이후 코드랑 상관 없이 메소드
        }

        /* 설명. 조건이 맞지 않아(회원이 조회되지 않아) 출력을 하는 구문(위의 조건이 맞으면 실행되지 않음) */
        System.out.println("슬플게도 우리 사이트는 아직 회원이 없습니다.");
